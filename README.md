...
Then open `dist/superagent-0.1.87.mcaddon` with Minecraft Education and activate both the behavior pack and resource pack in the world. A world owner or teacher must activate the pack first. For the full block set, the world must have Cheats enabled and students should run as Operator in Survival.

Local build output only (do not upload .mcaddon to the repo):
`dist/superagent-0.1.87.mcaddon`
...
```text
https://github.com/numraise/raisesuperagent#superagent-0.1.87
```
